...
.sb-logo { 
    font-size: 1.1rem; font-weight: 700; color: #ffffff !important;
    letter-spacing: -0.02em;
}
.sb-sub { font-size: 0.75rem; color: #64748b !important; }
.sb-hdr {
...
.dose-chip {
    ...
    font-size: 0.78rem; color: #93c5fd !important;
...
.badge {
    ...
    padding: 3px 10px; border-radius: 20px; margin-right: 5px;
...
.cv-blue   { color: #2166ac; }
.cv-red    { color: #dc2626; }
.cv-green  { color: #16a34a; }
.cv-orange { color: #d97706; }
...
.stButton > button {
    background: linear-gradient(135deg,#2166ac,#1d4ed8);
    color: white; border: none; border-radius: 8px;
    font-weight: 600; width: 100%; padding: 0.55rem;
    box-shadow: 0 2px 8px rgba(33,102,172,0.35);
}
.stButton > button:hover { background: linear-gradient(135deg,#1a5490,#1e40af); }
</style>
...
# STUDY DESIGN
# ============================================================
WEGOVY_LEVELS = [0.25, 0.5, 1.0, 1.7, 2.4]   # mg per block

COHORT_CONFIG = {
    "Reference": {
        "skip_block": None, "dwj_block": None, "dwj_dose": 0.0,
    },
    "Cohort I (W-W-T-W-W)": {
        "skip_block": 2, "dwj_block": 2,
        "dwj_dose": WEGOVY_LEVELS[2] * 8,   # 8.0 mg
    },
    "Cohort II (W-W-W-T-W)": {
        "skip_block": 3, "dwj_block": 3,
        "dwj_dose": WEGOVY_LEVELS[3] * 8,   # 13.6 mg
    },
    "Cohort III (W-W-W-W-T)": {
        "skip_block": 4, "dwj_block": 4,
        "dwj_dose": WEGOVY_LEVELS[4] * 8,   # 19.2 mg
    },
}
...
#   CL, Q         : L/h
#   ka, kdr, kout : h⁻¹
#   IC50, EC50_AE : mg/L  (same unit as C)
# ============================================================
P = dict(
    V         = 25.0,
    Cl        = 3.5,
    Ka        = 5.2,        # h⁻¹  FR → A1 (fast-release LAI)
    ka_SC     = 1.0,        # h⁻¹  R  → A1 (Wegovy SC)
    F_SC      = 0.9,        # Wegovy SC bioavailability
    Scale_LAI = 0.2,        # DWJ1691 LAI bioavailability scaling
    F_DR      = 0.2,        # delayed-release fraction of LAI
    # F_FR = F_SC - F_DR = 0.7
    kdr       = 1.0,        # h⁻¹  3-transit delayed-release rate
    BW0       = 100.0,      # kg  (fixed baseline)
    Imax      = 0.21,
...
#
# PML dosepoint 처리:
#   dosepoint(R,  bioavail=F_SC)             → Wegovy dose×F_SC → R depot
#   dosepoint(FR, bioavail=Scale_LAI×F_FR)   → DWJ dose×Scale×F_FR → FR depot
#   dosepoint(DR, bioavail=Scale_LAI×F_DR)   → DWJ dose×Scale×F_DR → DR depot
#
# ODE deriv (PML 원본 순서):
#   dA1  = -(Cl×C) + (ka_SC×R) + (FR×Ka) + (DR3×kdr)
#   dFR  = -(FR×Ka)
#   dDR  = -(DR×kdr)
#   dDR1 = (DR×kdr)  - (DR1×kdr)
...
#   dR   = -(R×ka_SC)
# ============================================================
def build_ode(p, dose_events_R, dose_events_FR, dose_events_DR):
    """
    dose_events_X : list of (t_h, amount_mg)
    bolus → 짧은 infusion으로 근사 (dur=0.5h) → 수치 안정성 확보
    """
    def add_bolus(events, t, dur=0.5):
        val = 0.0
    ...
    def ode(t, y):
        ...
        dA1  = -(p['Cl'] * C) \
               ...
               + (FR * p['Ka']) \
               + (DR3 * p['kdr'])
        dFR  = -(FR * p['Ka'])   + add_bolus(dose_events_FR, t)
        dDR  = -(DR * p['kdr'])  + add_bolus(dose_events_DR, t)
        dDR1 =  (DR  * p['kdr']) - (DR1 * p['kdr'])
        dDR2 =  (DR1 * p['kdr']) - (DR2 * p['kdr'])
        dDR3 =  (DR2 * p['kdr']) - (DR3 * p['kdr'])
        dR   = -(R * p['ka_SC']) + add_bolus(dose_events_R, t)

        # ── BW PD (Indirect Response, sigmoidal Imax) ──
        # C [mg/L],  IC50 [mg/L]  → 단위 일치 ✓
        C_safe = max(C, 0.0)
...
def build_wegovy_events(skip_block=None):
    """
    Wegovy SC:  dose × F_SC  → R depot  (주1회, 4주 블록 × 5단계)
    """
    events = []
    for blk in range(5):
        if blk == skip_block:
            continue
        amt = WEGOVY_LEVELS[blk] * P['F_SC']   # bioavail 적용
        for w in range(4):
            t_h = (blk * 28 + w * 7) * 24.0
            events.append((t_h, amt))
    return events

def build_dwj_events(dwj_block, dwj_dose_mg):
    """
    DWJ1691 LAI: 1회 SC 주입 → FR depot + DR depot 으로 분리
      FR 주입량 = dose × Scale_LAI × F_FR
      DR 주입량 = dose × Scale_LAI × F_DR
    """
    if dwj_block is None or dwj_dose_mg <= 0:
        return [], []
    t_h   = dwj_block * 28 * 24.0
    F_FR  = P['F_SC'] - P['F_DR']          # 0.9 - 0.2 = 0.7
    amt_FR = dwj_dose_mg * P['Scale_LAI'] * F_FR
    amt_DR = dwj_dose_mg * P['Scale_LAI'] * P['F_DR']
...
def simulate_cohort(coh_name, p):
    ...
    t_end  = SIM_WEEKS * 7 * 24.0           # 28주 = 4704 h
    ...
    if not sol.success:
        st.error(f"ODE solver failed for {coh_name}: {sol.message}")
        return None

    t_h    = sol.t
    C_mgL  = np.clip(sol.y[0], 0.0, None) / p['V']
    C_ugL  = C_mgL * 1000.0                 # µg/L (표시용)
    BW     = np.clip(sol.y[7], 0.0, None)
    BW_pct = (BW - p['BW0']) / p['BW0'] * 100.0   # % change from BW0=100

    # GI AE: E = E0 + Emax×C/(EC50+C),  C in mg/L
    GI = np.clip(
        p['E0_AE'] + p['Emax_AE'] * C_mgL / (p['EC50_AE'] + C_mgL + 1e-15),
        0.0, 1.0) * 100.0

    return dict(t_h=t_h, C_mgL=C_mgL, C_ugL=C_ugL, BW_pct=BW_pct, GI=GI)
...
# ============================================================
with st.sidebar:
    st.markdown('<div class="sb-logo">💊 PK/PD Simulator</div>',
                unsafe_allow_html=True)
    st.markdown('<div class="sb-sub" style="margin-bottom:8px">DWJ1691 + Wegovy · v1.0</div>',
                unsafe_allow_html=True)
    ...
    dose_info = {
        "Cohort I":   ("W-W-<b>T</b>-W-W", "8.0 mg",  "wk 9"),
        "Cohort II":  ("W-W-W-<b>T</b>-W", "13.6 mg", "wk 13"),
        "Cohort III": ("W-W-W-W-<b>T</b>", "19.2 mg", "wk 17"),
    }
    for name, (pattern, dose, timing) in dose_info.items():
        st.markdown(
            f'<div class="dose-chip">'
            f'{pattern} &nbsp;→&nbsp; <b>{dose}</b> at {timing}'
            f'</div>',
            unsafe_allow_html=True
        )
...
<div class="main-header">
  ...
  <div class="design-strip">
    <div class="design-item">
      ...
      <span>Wegovy: 0.25→0.5→1.0→1.7→2.4 mg q1w (4wk each)</span>
    </div>
    <div class="design-item">
      <div class="design-dot" style="background:#2166ac"></div>
      <span>Cohort I: DWJ <b>8.0 mg</b> @ wk9</span>
    </div>
    <div class="design-item">
      <div class="design-dot" style="background:#16a34a"></div>
      <span>Cohort II: DWJ <b>13.6 mg</b> @ wk13</span>
    </div>
    <div class="design-item">
      <div class="design-dot" style="background:#dc2626"></div>
      <span>Cohort III: DWJ <b>19.2 mg</b> @ wk17</span>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
...
with k1:
    st.markdown(f"""<div class="kpi-card kpi-blue">
      ...
      <div class="kpi-value cv-blue">{np.max(all_C):.3f}</div>
...
with k3:
    st.markdown(f"""<div class="kpi-card kpi-green">
      ...
      <div class="kpi-value cv-green">{np.min(all_bw):.2f}%</div>
    ...
    for coh, r in results.items():
        ...
        fig_bw.add_trace(go.Scatter(
            ...
            hovertemplate=(
                f"<b>{coh}</b><br>"
                "Time: %{x:.0f} h<br>"
                "ΔBW: %{y:.3f}%<extra></extra>"
            )
        ))
    fig_bw.add_hline(y=0, line_dash="dot",
                     line_color="#cbd5e1", line_width=1.2)
...
with col_gi:
    ...
    fig_gi.update_layout(
        **CHART_BG, height=340,
        ...
        yaxis=dict(**CHART_BG['yaxis'], range=[0, 100]),
        showlegend=False
    )
...
for coh, r in results.items():
    ...
    rows.append({
        ...
        "Max ΔBW (%)":      round(float(np.min(r['BW_pct'])), 3),
        "Peak GI AE (%)":   round(float(np.max(r['GI'])),     2),
    })
...
st.dataframe(df, use_container_width=True, hide_index=True,
    column_config={
        ...
        "Max ΔBW (%)":       st.column_config.NumberColumn(format="%.3f"),
        "Peak GI AE (%)":    st.column_config.NumberColumn(format="%.2f"),
    })
...
with st.expander("📋 Model Parameters (Phoenix NLME fixef)"):
    ...
    with c3:
        ...
        st.markdown("---")
        st.markdown("**Dose Bioavailability**")
        st.markdown(f"- Wegovy R depot: dose × {P['F_SC']}")
        st.markdown(f"- DWJ FR depot: dose × {P['Scale_LAI']} × {P['F_SC']-P['F_DR']:.1f}")
        st.markdown(f"- DWJ DR depot: dose × {P['Scale_LAI']} × {P['F_DR']}")
